...
- [x] Current 45,395-byte rank-only revision reproduced and locally verified:
...
- [x] Local factual-paraphrase proxy passes 88 synthetic pairs with minimum
  margin `0.206250` and synthetic ordinal Spearman `0.958919`.
...
- [~] The live Spearman threshold is still unobserved because the retained result
  reports zero historical rows. Synthetic ordinal Spearman `0.958919` is a proxy only.
