...
- **Current state:** The local repository now passes 117 Python tests, nine payment-canary tests, compilation, and canonical-YAML draft validation. Discovery uses live score/price units; Miner 212 and 18 have schema-specific adapters and comparability gates; service validation, event identity, proxy-aware rate limiting, and Caddy forwarding are hardened. The public decision UI is deployed as a separate fail-closed service behind Caddy: root and health respond publicly, while `/api/decision` returns 503 until the reviewed Telegraph-backed runner/payment path is configured. Release `oathcast-2026-08-09-readiness-v4` is deployed at `https://oathcastcourt.duckdns.org`; public health/readiness report source digest `d3069ec17f6fc7f24224b3ebc3803e665500f24f9e71fd948fbc98b9b20de233` and image digest `sha256:6cd62e074ccd33aa1233e6e750d9163bcd09870981c05a92045ecd3938e6a66a`. The v3.2 container is retained stopped as `oathcast-v3-2-rollback-20260809`. The current Solana-devnet x402 challenge for Miner 18 passed a non-signing 0.01-USDC preflight, and the manually authorized capped paid canary then confirmed on-chain with an exact 10,000-base-unit payer-to-recipient movement. Sanitized evidence is archived; the latest Explorer recheck at `2026-08-09T21:52:34Z` shows live weather traffic for Miners 212 and 18 but still no one-to-one record for the canary transaction, so it remains non-qualifying evidence. A dedicated devnet-only wallet is funded with 20 test USDC in owner-only, gitignored `.secrets/`; no live-value funds are involved. Temporary direct SSH was removed after deployment verification; the AWS security group now exposes HTTP/HTTPS only. No Miner registration or WASM registration exists. The published portal parser/wizard was audited locally and the draft is shape-compatible; official runtime validation, a unique Integration ID, pinning, and registration remain blocked until the Hackathon registration surface opens/finalizes. Details are in [`docs/official-portal-compatibility.md`](docs/official-portal-compatibility.md).
...
### Advisor review — chronological backtest (2026-08-05)

The advisor returned **REVISE** for the initial issued-time-only design. The
backtest now gates prior history on `resolved_at <= current issued_at`, batches
equal issue times, freezes the warmup/holdout policy and tie-breaks, rejects
mixed horizons and duplicate IDs, separates conditional Brier from coverage
and end-to-end utility, reports common-case Brier, and excludes unresolved
events from scoring and future history. Missing, late, invalid, and abstained
attempts contribute zero utility rather than zero Brier loss.
...
- Ahmed's 2026-08-03 Discord answer named Base Sepolia USDC. Preserve that as historical guidance, but do not use it to sign current consumption requests: the live API challenge observed on 2026-08-09 now requires Solana Devnet USDC. Registration and consumption can use different test networks.
...
- No payment is needed for local fixtures, service tests, YAML drafting, Script Author work, or offline Application logic.
...
- **Current canary path:** the live API now offers Solana Devnet USDC. `payment-canary/` is the authoritative implementation for this route; the Python Base-Sepolia boundary is legacy regression code only. One unpaid preflight passed, followed by one manually authorized capped payment at 0.01 USDC. Solana RPC confirmed the transaction and exact token movement; the Explorer demand record is not yet reconciled.
- **Implement during Track 3 (from 2026-08-31):** enable the reviewed Application runner, use the dedicated faucet-funded Solana-devnet wallet, send legitimate paid requests through Telegraph, and verify each resulting Miner activity record in the Explorer. The Aug 9 canary is a one-request integration check, not qualifying Track 3 demand. Track 3's official window is Aug 31–Sep 7, 2026.
...
- A devnet-only wallet was generated locally on 2026-08-09 because the user explicitly authorized one capped canary. Circle faucet funding produced 20 test USDC; one 0.01-USDC canary was executed successfully. Its secret is owner-only and gitignored. Do not copy it to the Miner host, reuse it on mainnet, or automate repeated payments.
- The current dispatcher is HTTP. The canary allows only the exact live authority with redirects disabled and a strict canonical gateway-prefix transformation. This temporary exception is acceptable only for the manually capped devnet canary and must be removed when Telegraph publishes HTTPS.
...
- Solana Devnet USDC has no financial value and is not backed by real dollars. A Telegraph payment is still a real devnet protocol/accounting transaction. Use only the dedicated test wallet and never use mainnet assets.
...
### Chronological provider backtest (2026-08-05)

The timestamped development fixture uses four warmup cases and a six-case
holdout. One warmup outcome resolves after a later forecast is issued, so the
selector cannot use it until its `resolved_at` timestamp. Equal-issued-time
cases are batched; unresolved cases are excluded from scoring and future
history. The report defines conditional Brier, coverage, common-case Brier,
and end-to-end utility separately.

| Provider | Holdout Brier | Coverage | End-to-end utility |
| --- | ---: | ---: | ---: |
| Open-Meteo | 0.061060 | 0.833333 | 0.782450 |
| OpenWeather One Call | 0.081780 | 0.833333 | 0.765183 |
| WeatherAPI | 0.093125 | 0.666667 | 0.604583 |

The prior-only selector chose Open-Meteo for all six holdout decisions and
achieved `0.782450` end-to-end utility. These are synthetic methodology
checks only; they do not establish provider superiority, live usage, or
qualifying Telegraph traffic.
...
5. The official Telegraph settlement/Explorer reconciliation path for the completed canary, plus a safe reviewed Application payment flow for legitimate Track 3 requests.
6. Exact submission artifacts and pre-build rules.
7. The final Hackathon 1 integration YAML, exact hash/schema-URI rules,
   credential/output fields, contract addresses, and testnet registration flow.
   The published portal parser/wizard was checked locally against the draft and
   the shape is compatible; the unique Integration ID, live portal validation,
   exact final bytes/hash, and wallet submission remain pending. Ahmed has
   explicitly removed the Machina bond from H1 contracts. See
   `docs/official-portal-compatibility.md`.
8. The eventual Explorer/API payment artifact and reconciliation format an
   ...
   but not yet released. A public API/leaderboard check on 2026-08-09 found
   Miner 18's existing epoch entry but no one-to-one record for the direct
   canary, so the canary remains non-qualifying evidence until the record can be
   matched or Telegraph documents the direct-call accounting view.
...
payment settled; the one-request canary has independent Solana RPC settlement
verification, while official Telegraph Explorer reconciliation and the
reviewed Application flow remain outstanding.
...
- Docker is installed and enabled. Active container `oathcast` runs image `oathcast:oathcast-2026-08-09-readiness-v4` with `--restart unless-stopped`, bound to `127.0.0.1:8080` (private to the instance) and durable host storage `/home/ec2-user/oathcast/data:/data/oathcast`; the separate `oathcast-decision-ui` container is bound to `127.0.0.1:8787` and remains fail-closed until the reviewed Telegraph-backed runner/payment path is configured. The v3.2 image is retained stopped as `oathcast-v3-2-rollback-20260809`; older rollback containers remain stopped as well. Container `oathcast-caddy` runs with host networking, persistent `/home/ec2-user/oathcast/caddy-data` and `caddy-config` mounts, and proxies public ports 80/443 to the Miner and UI. The remote owner-only env file is `/home/ec2-user/oathcast/.env`; it now holds the active rotated key. The owner-only local backup at `/Users/femi/Downloads/oathcast-miner.env` retains the retired key for the stopped v3.1 rollback only and is not a current smoke credential. Never print, commit, or paste either file's contents.
...
- **2026-08-05:** Superseded the repository-pending note: created the public GitHub repository at `https://github.com/fexx301/oathcast`, pushed `main`, enabled secret scanning and push protection, added a CI matrix for Python 3.11/3.12, and added `SECURITY.md` plus expanded local-secret ignores. The canary's `OATHCAST_MINER_API_KEY` Actions secret remains intentionally unconfigured; scheduled runs skip authenticated checks safely until the active staging credential is deliberately added.
- **2026-08-09:** Reconciled the project against the released API/boilerplate surface. Fixed live Miner discovery score and price units, added explicit WeatherAPI Miner 212 and Zeus Miner 18 request/response adapters, corrected the canonical YAML's consumer-facing parameter map, hardened service validation/rate-limit identity/event IDs, and added a fail-closed public decision UI. The full Python suite passes 117 tests.
- **2026-08-09:** Added a separate official-SDK Solana x402 canary with strict network/mint/amount/recipient/fee-payer/route checks, one-shot execution, sanitized evidence, and independent Solana RPC settlement verification. The live Miner-18 unpaid preflight passed for exactly 0.01 Solana Devnet USDC; the subsequent manually authorized canary confirmed with a 10,000-base-unit payer-to-recipient movement. Circle and Solana official documentation confirm these devnet tokens have no monetary value. Telegraph Explorer reconciliation is still pending.
- **2026-08-09:** Generated one dedicated devnet-only wallet at `.secrets/solana-canary.env`; the file is mode 0600, ignored by Git, and was never printed. Circle faucet funding left it with 20 test USDC; no mainnet funds are present. Prepared and deployed release `oathcast-2026-08-09-readiness-v4` with source digest `d3069ec17f6fc7f24224b3ebc3803e665500f24f9e71fd948fbc98b9b20de233` and bundle digest `147046dff2720f3a15ac50344036efa94d50d739431dc517206adf6d3f9bf254`; temporary direct SSH was opened only for deployment and removed afterward.
- **2026-08-09:** Queried Telegraph's public recent-signal feed and Miner leaderboard after the canary. The feed did not expose a matching transaction/signature record; the leaderboard exposed an existing `bittensor-sn18-zeus` entry but not a one-to-one canary record. The sanitized canary artifact remains explicitly non-qualifying until the Explorer record is matched.
- **2026-08-09:** Deployed `oathcast-2026-08-09-readiness-v4` to the authorized EC2 host behind Caddy. Public `/healthz`, `/readyz`, UI root/health, unauthenticated rejection, fail-closed `/api/decision`, and an authenticated near-term forecast all passed; release evidence is in `artifacts/release-evidence/public-smoke-2026-08-09-v4.json`. The development question fixture targets beyond the seven-day Open-Meteo adapter window, so it is not a valid deployment smoke input. The old v3.2 container remains stopped for rollback. AWS security-group cleanup was completed immediately afterward; only HTTP/HTTPS remain publicly reachable.
- **2026-08-09:** Final local verification passed 117 Python tests and nine payment-canary tests. The implementation and release evidence were committed as `3540246` (`Harden Telegraph integration and deploy readiness v4`) and pushed to `https://github.com/fexx301/oathcast`; the pre-existing untracked `improvements.md` was deliberately left untouched. The next session should begin with Explorer reconciliation and the official registration/harness surfaces, not another deployment or payment attempt.
...
- **2026-08-10:** Telegraph answered the three open Discord questions. (1) Each signal gets a **unique hash searchable on the Explorer**; the feature was under development and going live EOD 2026-08-10. This explains the negative 168-hour lookback: the attribution path did not exist yet, so the absence of a canary record was **not** evidence of a broken integration, and the archived `explorer_reconciliation.status: "not_found_yet"` must not be described as a failure. (2) Devnet rehearsal **does** count as Track 3 preparation — "same HTTP requests be it on testnet or mainnet" — so the Solana-devnet canary is a valid rehearsal of the request path, though still not qualifying demand. (3) **The paying wallet is the unique identifier**; no consumer-side registered identity, key, or custom header is needed, which removes an assumed blocker and makes consumer-identity work dead scope. The 100-real-request and 3-active-Miner guardrails are unchanged. Recorded as §C0 in `improvements.md` with actions T1 (re-run Explorer reconciliation once the hash ships), T2 (capture the per-signal hash in canary evidence), and T3 (drop consumer-identity scope).
- **2026-08-10:** **Corrected a strategic misallocation.** The improvement register's top Track 1 item was a calibration layer (isotonic regression over raw probabilities) to improve Brier. But Brier is not part of Telegraph's current scoring model — the live scorer is a `0..1` composite over cosine similarity, BM25 word overlap, and response-length quality, all computed on the **response text** (see §10 and the 2026-08-03 Ahmed clarification). A calibration layer would therefore have improved a number nobody is scoring. Since Miner performance is 75% of Track 1 and is scored on text, `src/oathcast/render.py` **is** the scoring surface. Calibration is demoted to an honesty/Track-2 item, not dropped.
- **2026-08-10:** Reworked the renderer (register item P0). The shipped v1 sentence scored **0.4424** on the local proxy — below the 0.55 good-response threshold — for two measurable reasons: ISO-8601 stamps tokenize into fragments (`2026`, `08`, `17t15`, `00z`) that match nothing a resolution text would contain, and the sentence never answered the yes/no question that was asked. `semantic_text_v2` now leads with IPCC AR6 calibrated uncertainty wording, states the window in readable UTC clock time, and keeps the exact percentage. v1 is retained as `render_forecast_content_v1` so the change stays measurable and reversible. Gains are **+0.05 on the F1 guard lane** and **+0.15 on the recall proxy**; both are local proxies, **not** Telegraph scores. Evidence in `docs/renderer-experiment.md` and `artifacts/renderer-benchmark/`. Covered by 10 tests including a 1001-point sweep asserting no anti-gaming issue trips at any probability.
- **2026-08-10:** **Two methodological errors were caught in the renderer experiment before they influenced the shipped result, and both are documented rather than quietly fixed.** First, the initially winning variant contained the phrase `during the requested UTC hour`, lifted from the same fixture corpus used to score it; an ablation proved the phrase contributed **exactly zero** on the F1 guard lane (0.5411 vs 0.5411; 0.5828 vs 0.5828), so the shorter non-borrowing variant was promoted instead. Second, the local proxy scores **recall only** (`|truth ∩ response| / |truth|`), so it rewards padding: a verbose variant topped the proxy at 0.6600 while ranking second-to-last on the F1 guard at 0.4889. Ranking was moved to the guard lane with a brevity tie-break, because cosine and BM25 both normalize for length and F1 is the closer analogue of Telegraph's actual scorer.
- **2026-08-10:** Closed the security batch. **Three register items were already implemented and the register was stale** — S1 (forwarded rate-limit identity is trusted only from a configured proxy network), S3 (`default_event_id` hashes every canonical question field, not just the start timestamp), and S6b (lat/lon finite-range validation). Newly implemented: **S6a** provider body cap at 2 MB, rejecting an over-cap declared `Content-Length` before reading any body and reading one byte past the cap so an at-limit body is accepted while a larger one is detected rather than silently truncated into a misleading JSON error; **S4** non-root container plus `HEALTHCHECK`; **S2** receipt-store capacity bound; **S5** receipt hash chain and external anchoring. Suite 94 → **170 tests**. None of this is deployed yet; the live host still runs v4.
- **2026-08-10:** **S4 nearly shipped a silent production outage.** The first Dockerfile draft used UID 10001. A bind mount preserves *host* ownership, and the durable EC2 directory `/home/ec2-user/oathcast/data` is owned by uid 1000 — so a container running as 10001 cannot write receipts, while `/healthz` and `/readyz` both still return 200. Every forecast would fail at persistence time with nothing in the health surface indicating why. This was initially masked by a testing artifact: Docker Desktop on macOS virtualizes ownership for named volumes and reported `WRITE OK` for a directory it claimed was owned by uid 1000. A kernel-level check with real uid separation returned `PERMISSION DENIED`. The image is pinned to `USER 1000:1000` and verified end to end (non-root identity, `/healthz` 200, `/readyz` 200, unauthenticated forecast 401, Docker health `healthy`, receipt DB created on the mount). **Changing this UID requires chown-ing the host directory in the same change**; this is recorded in the Dockerfile, `DEPLOYMENT.md`, and `improvements.md`.
- **2026-08-10:** S2 bounds receipt-store growth with a **capacity cap rather than a retention policy**. Deletion-based retention was considered and rejected: receipts exist to be replayed after cutoff, so evicting them would destroy the property they provide, and the DELETE trigger would reject it anyway. New receipts past 200,000 rows / 512 MiB (both overridable, either disable-able) return HTTP **507**; the service fails closed rather than serve a forecast it cannot record, because an unrecorded forecast cannot be replayed or verified. **A replay of an already-issued receipt always succeeds even at capacity** — the Miner has publicly committed to those events. `/readyz` reports capacity and goes 503 when full while `/healthz` stays 200, so a capacity stall is diagnosable instead of becoming a Docker restart loop. The canary fails at <10% headroom, before the cliff. Two bugs were found while implementing it: `ReceiptStoreFull` was being swallowed by the provider loop's `except Exception` and surfacing as a **502** (sending an operator to debug an upstream provider for a local disk problem, and re-fetching upstream once per remaining provider), and the `:memory:` connection never had `row_factory` set, so every name-based column access on an in-memory store raised `TypeError` — latent because no prior test used one.
- **2026-08-10:** S5 makes receipts tamper-**evident**. The gap: anyone with the SQLite file can rewrite a receipt *and* recompute a valid `receipt_sha256`, producing a self-consistent forgery that every per-receipt check accepts — triggers only stop the SQL path. Added `chain_head()` (`chain[i] = sha256(chain[i-1] || event_id || recomputed_digest)`, ordered by `rowid`) and `scripts/anchor_receipt_head.py` with `--output`/`--verify`, exiting non-zero on mismatch. Three design points are load-bearing: a **chain** rather than a set digest, so a head published at N receipts stays verifiable forever as the store grows (a set digest would go stale on every new receipt and nobody would keep checking it); ordering by **`rowid`, not `created_at`**, because wall-clock ordering lets a replayed or clock-skewed receipt reorder an already-published prefix and break a valid anchor; and chaining the **recomputed** digest rather than the receipt's own field, which is the entire point. `receipt_digest` moved into `receipts.py` so store and service hash identically, and `get()` now raises `ReceiptTampering` when stored bytes disagree with a present digest field (receipts with no digest field stay readable — refusing them would turn a missing feature into data loss). **The anchor is only worth anything once published where OathCast cannot rewrite it**; an anchor file living only in this repo proves nothing against someone who can also edit the repo. 16 tests, including a self-consistent forgery that defeats the per-receipt check but still moves the head, and a truncated store that must not verify against a shorter prefix.
- **2026-08-10:** Chose **WeatherAPI over OpenWeather** as the P4 failover candidate, on cost rather than quality. WeatherAPI's free tier needs no credit card and issues a key instantly. OpenWeather One Call 3.0 — which `adapters/openweather.py` targets — requires a card-backed subscription, and its daily cap **defaults to 2,000 rather than the free 1,000**, so it can bill an operator who did nothing wrong; OpenWeather's genuinely card-free tier is the classic 3-hour forecast API, which lacks the hourly `pop` field the adapter reads. Deferred under the standing no-new-cost rule. A live WeatherAPI key was provisioned into `.secrets/weatherapi.env` (mode 0600, gitignored) and verified with one call: Lagos parsed cleanly through the existing adapter with **no code change needed**.
- **2026-08-10:** **A working key does not close P4, and the gate at `service.py:396` stays shut.** WeatherAPI's `chance_of_rain` has unstated threshold semantics, and there is a concrete reason to doubt equivalence visible in the adapters: `open_meteo.py:62` selects the point at **`horizon_end`** (its probability is documented as ">0.1 mm in the *preceding* hour") while `weatherapi.py:88` selects at **`horizon_start`** (WeatherAPI labels an hourly block by the hour it begins). Both match their own provider's docs, but if either reading is wrong the two providers are silently answering about **different 60-minute windows**. The first collected pair showed **open_meteo 0.01 vs weatherapi 0.13** for the same Lagos hour — a 13x divergence against a 0.2305 climatology, in case one. This is exactly what a held-out test catches and a spot-check does not.
- **2026-08-10:** Built `scripts/collect_provider_pairs.py` for the P4 equivalence evidence. Neither free tier sells a historical *forecast* archive, so the comparison can only accumulate **forward** — a key obtained on 2026-08-16 yields one day of data before Track 1 opens on 2026-08-17, which is why this is the one remaining item that cannot be compressed later. `collect` appends one unresolved case per location at a fixed lead time; `resolve` fills outcomes from an independent observation export once the window has closed. Output uses the schema `backtest_providers.py` already consumes, verified by loading a written dataset through `load_chronological_cases`. Load-bearing choices: the key is read **only** from `WEATHERAPI_KEY` in the environment, never an argument (arguments land in shell history and process listings); a failed provider is recorded as `status: "missing"` rather than dropped, because dropping would bias the comparison toward whichever provider is more available; a changed lead time is refused without `--allow-lead-change`, because providers compared at different lead times measure lead time; writes are atomic and validated through the real loader before `os.replace`, because a scheduled job that installs an unloadable dataset fails at analysis time; an open window is never resolved (leakage) and resolution is idempotent. 23 tests, none touching the network.
- **2026-08-10:** **The secret-scrubbing test was initially passing for the wrong reason.** A live check with a corrupted key confirmed the key did not appear in the error — but only because urllib's `HTTPError: HTTP Error 401` message happens to omit the URL. urllib **does** copy the request URL into connection-failure messages (DNS failures, timeouts), and the WeatherAPI key is a query parameter, so a scheduled job hitting a network blip would have written the key into its log. The regression test was rewritten to raise from `fetch_json` with the built URL in the message and to assert `<redacted>` is **present**, so the scrub cannot be silently removed and cannot pass by luck.
- **2026-08-10:** **Refused an unsourced climatology and derived a real one.** `load_locations` rejects `climatology_source: "UNSET"` because Brier skill is measured against that baseline — scoring against a number nobody derived is a fabricated result. The 0.35 in `fixtures/brier_cases.json` is an unsourced development value. The frozen replacement is **0.2305**, derived from ERA5 via Open-Meteo's archive API: all 7,440 August hours 2015-2024 at the Lagos point, 1,715 above 0.1 mm, matching the question's own threshold. Two caveats are recorded in `fixtures/collection_locations.json` rather than hidden: ERA5 is ECMWF reanalysis retrieved **through Open-Meteo**, so it is *not fully independent of the `open_meteo` provider* and that limits how much weight the skill number can carry; and the annual rate ranges 0.1089 (2020) to 0.3602 (2021), so a single-season sample would have been unrepresentative. **P4 still needs an independent observation export** — the bundled one is a fixture whose independence is not asserted — before any collected case can be honestly resolved.

- **2026-08-10:** **Pushed.** Three commits (`74c3c2b` renderer v2, `d214444` security batch, `d48d13f` collector) and `1c218c0` (collection workflow); CI green on 3.11 and 3.12. Before this, 24 files — the entire renderer and security batch — existed only on one laptop. Split into three commits rather than one so the renderer stays independently revertible, as the docs promise. The commit set was scanned against the live values in `.secrets/` before pushing: the WeatherAPI and Solana values appear in **0** of the 24 files. One commit message was amended pre-push because it claimed the engine-routing question was "still open with the team" — the team answered it on 2026-08-10, and a commit message asserting the opposite of the record is worse than no message.
- **2026-08-10:** **The public canary is green and blind — 96 consecutive successes that verified nothing.** `gh run view` on its step list shows "Verify public Miner" **`skipped`** on every scheduled run, because `OATHCAST_MINER_API_KEY` was never configured as a repository secret and the workflow's guard step then reports success having checked nothing. The skip-safely pattern is right, but it means *the absence of a secret is indistinguishable from a passing canary at the run level*. The lesson generalizes to the new collector, which uses the same pattern: **a green workflow is not evidence; the evidence is the step conclusion.** Both are now documented with an explicit "confirm the step says `success`, not `skipped`" check. Fix requires setting the secret — an operator action, since the key must not pass through an agent session.
- **2026-08-10:** Scheduled P4 collection on **GitHub Actions**, hourly, writing to the `data/provider-pairs` branch. Chose it over EC2-only for a measured reason and a scheduling one. Measured: GitHub delivers scheduled runs best-effort — on this repository over 2026-08-06..10 the `*/15` canary received **96 of 409 requested runs (23%)**, with gaps up to **360 minutes**, which would silently lose two slots of a 3-hourly job. The fix is to over-request: `case_id` floors `issued_at` to the hour, so extra runs **converge on one case** and cannot double-count; at the observed rate hourly yields ~39 cases by Track 1 instead of ~13. Scheduling: EC2 needs an authorized SSH window, and the redeploy needs one anyway, so the host leg should be installed inside that window rather than opening port 22 twice. **Both legs can run** — the merge dedupes by `case_id` and their failure modes are uncorrelated (GitHub queue load vs host death). Two bugs were caught by rehearsing the job locally before pushing: `[ -s f ] && cat f` returns 1 on an empty file and would have failed the step under `set -e` on a *clean* run, and a `--depth=1` fetch can be refused on push with "shallow update not allowed". Verified by live dispatch: skip path green, collect step correctly `skipped`, and a same-hour re-run returns `cases_added: 0` with nothing pushed.
- **2026-08-10:** ~~**The host still serves the 2026-08-04 v3.2 image**, confirmed by `/readyz` returning ready with **no release ID**.~~ **SUPERSEDED the same day — see the correction entry below. The host runs `oathcast-2026-08-09-readiness-v4`.** The rest of this entry stands: renderer v2 — the Track 1 lever, 75% of that track's score — is currently worth **zero**, because scoring reads the live response text and no deployed release contains it. Also established that **the redeploy requires SSH**: every image the host has run was built on the host from local tags, with no registry to pull from, so source must reach the host and build there. This corrects an earlier assumption in this session that the GitHub collector leg avoided the SSH question entirely — it avoids it for *collection*, not for the deploy. Release manifest for `2026-08-10-hardened-v5` built and confirmed reproducible across runs: `source_sha256 = 8b1788ca…a10d46`, 56 files. Re-run it if any tracked file changes before the build; the digest covers the tree, not the release name.
- **2026-08-10:** **Correction — the live release is v4, not v3.2, and the error was a JSON-shape mistake of mine.** `/readyz` nests release identity one level down; I read `release_id` at the top level, got nothing, and reported the absence as "no release ID," which then propagated into `DEPLOYMENT.md` and the entry above. The host actually serves `release_id oathcast-2026-08-09-readiness-v4`, `source_sha256 d3069ec17f6fc7f24224b3ebc3803e665500f24f9e71fd948fbc98b9b20de233`, `image_digest sha256:6cd62e074ccd33aa1233e6e750d9163bcd09870981c05a92045ecd3938e6a66a` — consistent with the 2026-08-09 deployment already recorded in §12 and at line 420. The operational conclusion does not move: v4 predates renderer v2 and the security batch, so v5 is still required and renderer v2 is still live-worth-zero. What moves is the gap size (one release, not two) and the rollback target (leave the **v4** container stopped, as v3.2 was left as `oathcast-v3-2-rollback-20260809`). **Read `/readyz` as `release.release_id`.** The general lesson matches the canary's: a missing field and a negative finding look identical unless you check the shape — the same failure mode as a green run that skipped its only real step.
- **2026-08-10:** **Collection is live and producing paired cases.** After the operator set the `WEATHERAPI_KEY` repository secret, an authorized dispatch collected against both providers; verified three independent ways rather than by the run's green tick — step conclusion `success` (not `skipped`), branch commit `71ee312` on `data/provider-pairs`, and case count 1→2 — then re-validated the branch dataset through the real `load_chronological_cases` (`2 cases, digest 613c488b52bc…`). A green tick alone was explicitly *not* accepted: an earlier run reported as passing was the pre-secret dispatch whose collect step read `skipped`. **Both collected pairs reproduce the P4 divergence in the same direction**: 12:00Z `open_meteo` 0.01 vs `weatherapi` 0.13, 16:00Z 0.00 vs 0.11. That is the signature expected if `open_meteo.py:62` selecting at `horizon_end` and `weatherapi.py:88` selecting at `horizon_start` are answering about different 60-minute windows — but both are dry calls against the 0.2305 climatology, so it is **not yet distinguishable from Lagos simply being dry**. Two cases prove nothing; the collector keeps running, and the verdict still waits on an independent observation export. **The same-hour dedupe is now proven live, not just in tests**: the first *scheduled* run (16:12Z, run `31407702336`) reached both providers — `valid_provider_attempts: 2` — and correctly reported `cases_added: 0`, "no new case this hour (already collected); nothing to push", because the 16:09Z dispatch had already taken that hour. That is the exact mechanism the dual-collector decision rests on, so over-requesting really is free coverage rather than duplication.
- **2026-08-10:** **`OATHCAST_MINER_API_KEY` is self-generated, not Telegraph-issued.** The `auth` block in `miners/oathcast-weather.yaml` (and the three provider drafts) only *declares* that clients send `Authorization: Bearer <value>`; the operator chooses the value. `service.py` consumes it fail-closed at `:274`/`:280`/`:288`. The owner-only copy at `/Users/femi/Downloads/oathcast-miner.env` was tested and is **the retired key** — `smoke_miner.py --token-env` returned `authenticated_forecast: 401` alongside `unauthorized_rejected: 401`, confirming it is the credential retired in the Aug 4 rotation. The value never entered the transcript or a process listing, because `--token-env` reads it from the environment by name. **Do not rotate under time pressure**: read the active value from `/home/ec2-user/oathcast/.env` during the SSH window and set it as the `OATHCAST_MINER_API_KEY` repository secret, which is what stops the canary being green and blind.
- **2026-08-10:** **The AWS console is the only route to the security group, and the region is `eu-north-1`.** No `aws` CLI is installed, `~/.aws` does not exist, and no `AWS_ACCESS_KEY_ID`/`AWS_PROFILE` is set, so there is no local credential fallback if the console session fails. An operator console screen showed "Unauthorized" during this session — session expiry, not an infrastructure fault; the host stayed up (`/healthz` 200) and the 16:00Z collection landed while the console was signed out, because the Actions leg is AWS-independent. Security groups, instances, and key pairs are **regional objects**: the console must be switched to `eu-north-1` (Stockholm) or `oathcast-web` will not appear at all. **A half-open security group plus an expiring console session is the worst possible state**, so start the SSH window only with a stable session and ~30 uninterrupted minutes, and re-check the public IP immediately before writing the /32 rule.

- **2026-08-10:** **v5 is deployed and is the first release whose scored surface is renderer v2.** `release_id 2026-08-10-hardened-v5`, `source_sha256 8b1788ca…a10d46`, `image_digest sha256:22a7c789…f96b3f`, running as `uid=1000(oathcast)` — v4 ran as **root**, so this is also the first release that drops privileges. Verified through public HTTPS with auth, not from inside the host: 200 with receipt `276ca07d0e5a64e9f64d5b1dbcd08a6e7177c68d22e6adbf845a3af13b5e281c` and the IPCC-calibrated text *"Measurable precipitation > 0.1 mm is very unlikely to occur in Lagos in the hour from 20:00 to 21:00 UTC on 10 August 2026. Probability: 0%."* This matters for Track 1 specifically: Telegraph scores the **response text** (cosine similarity + BM25 overlap + length quality), so the renderer is the scoring lever and it is now live. The 5 pre-existing receipts survived the cutover; v4 is kept stopped as `oathcast-v4-rollback-20260810` and a byte-verified backup sits at `/home/ec2-user/oathcast/receipts-preV5-backup.sqlite3`. `oathcast-decision-ui` was deliberately left on v4 rather than cutting both at once.
- **2026-08-10:** **A bind-mount UID bug would have shipped a Miner that returns 200 on both health endpoints while every forecast fails to persist.** v5 drops to `uid=1000`; a bind mount preserves *host* ownership, and `/healthz` and `/readyz` do not touch the receipt store, so the failure is invisible to exactly the checks meant to catch it. `DEPLOYMENT.md` said to `chown` the **directory** — the directory was already `1000:1000 mode 700`, so following the runbook literally would have looked like compliance and still broken. The file inside it was `root:root 644`. **Two probes disagreed and the reassuring one was wrong**: `test -w` reported the file writable and a `BEGIN IMMEDIATE` appeared to succeed, while a real `CREATE TABLE` against an exact-ownership byte copy failed with `OperationalError: attempt to write a readonly database`. Fixed with `chown -R 1000:1000` on the data directory *before* starting the new container; production receipts were never used as the test subject. Lesson recorded in `DEPLOYMENT.md`: for SQLite write access, trust only an actual SQLite write, and check the **file**, not the directory.
- **2026-08-10:** **The host collector runs on a systemd timer, not cron — Amazon Linux 2023 ships without `cronie`.** `crontab -e` returns `command not found` and `crond` is inactive, so the runbook's cron line was unexecutable as written (now corrected in `docs/p4-host-collection.md`). A timer needs no new package or daemon, and `Persistent=true` fires a run missed while the instance was down — which cron cannot, and which matters because **a missed collection hour is permanently unrecoverable**: no free tier sells a historical *forecast* archive. Confirmed on the **scheduled** path, not just by hand: the 18:07Z timer firing produced `lagos-20260810T1800Z` with `valid_provider_attempts: 2`.
- **2026-08-10:** **The two collection legs agree exactly, which is a stronger result than the dedupe it was checked for.** Merging the host's 2 cases into the branch's 4 added **zero** (`case_id` dedupe confirmed on real data from two independent collectors). More usefully, every overlapping provider value was *identical* — 17:00Z `open_meteo` 0.0/0.0 and `weatherapi` 0.13/0.13; 18:00Z 0.01/0.01 and 0.14/0.14 — across different networks and clocks. So a case's value does not depend on which leg wrote it, which is the precondition for treating the legs as interchangeable. It also confirms the **P4 divergence is a real provider disagreement, not collection noise**, since the 0.13-vs-0.01 gap reproduces across independent collectors. It still does not distinguish the `horizon_start`/`horizon_end` selection bug from Lagos simply being dry.
- **2026-08-10:** **The release manifest includes `.env.example`, which is why a correct source tree can still fail its digest check.** The host reproduced `a5badc5a…` against the pinned `8b1788ca…`. Rather than assume a sync error, `create_release_manifest.py`'s `INCLUDE` tuple was read and the two manifests diffed: all 55 shared files matched byte-for-byte and only `.env.example` was absent, because the sync excluded dotfiles. After confirming it carries empty values for every secret-bearing key and that the real `.env` (mode 600) was untouched, copying it reproduced the pinned digest exactly. Also: **macOS rsync 3.4.0 crashes** with `buffer overflow: recv_rules (exclude.c:1683)` on this transfer, and the `rsync exit=0` printed alongside it was `tail`'s status, not rsync's — the transfer was redone as tar-over-SSH with digest verification on both ends.
- **2026-08-10:** **First receipt anchor written: head `8a63dba5…40e230` over 6 receipts, `integrity_check: ok`, no self-reported digest mismatches.** Pulled to `artifacts/receipt-anchors/anchor-2026-08-10.json` and verified identical on both ends (`ad116b0f…e835c4`). **This is a prefix commitment, not yet a proof** — it currently lives only on the host and in this repo, both controlled by the same party that writes receipts, so it demonstrates the chain is internally consistent and nothing more. It becomes evidence only once a copy is published somewhere append-only and third-party observable.
- **2026-08-10:** **SSH window 17:04:57Z → closed after 18:13:02Z; 58 accepted sessions from a single IP, 0 failed or invalid-user attempts.** Read from the host's `sshd` **journal**, because `/var/log/secure` is empty on AL2023 — checking that file reports a reassuring zero that means nothing. Zero failures is the /32 scoping working: scan traffic never reached `sshd`. Everything requiring SSH was completed inside the window — v5 build and cutover, collector install, receipt anchor, and pulling back both the anchor and the dataset — so no follow-up reopening is owed.
- **2026-08-10:** **Both repository secrets are now set** (`WEATHERAPI_KEY` 15:54:46Z, `OATHCAST_MINER_API_KEY` 17:57:36Z), which **supersedes the "do not rotate under time pressure" pending action in the 2026-08-10 key-provenance entry above.** The active Miner key was read from `/home/ec2-user/oathcast/.env` and piped host → `gh secret set` without being rendered into this session, a terminal, or a process listing. The canary is no longer green-and-blind on auth.
- **2026-08-10:** **The canary's fixed-date fixture made it a guaranteed false alarm, and would then have become a permanent one.** `fixtures/question.json` pins 2026-08-17T15:00Z, which sits **past Open-Meteo's rolling 7-day window today** (`select_exact_point` correctly refuses to substitute a neighbouring hour → `provider_unavailable` → 502) and **past its own `forecast_cutoff` after 2026-08-17T12:00Z** (`service.py:435` rejects `now >= cutoff`). So it fails now, works for six days, then fails forever — and a genuine outage would have looked identical. `smoke_miner.py` now computes a rolling horizon (12:00–13:00 UTC tomorrow: lead 12–36h, cutoff always ≥11h future) instead. **Anchoring to the next UTC day rather than `now + N hours` is deliberate**: the receipt hash derives from the canonical question, so a per-run horizon would have made each of the 96 daily canary runs a distinct question and written **96 synthetic receipts a day** into the store that is supposed to be evidence of *real* demand. Stable within the day, it costs at most one. Covered by 5 new boundary tests; 198 tests pass.

- **2026-08-10:** **The same stale-fixture bug was also in `preflight_miner.py`, found by asking who else reads the fixture.** After fixing the canary, a sweep of `fixtures/question.json` readers turned up two more live callers. `demo_application.py` is harmless — its probabilities are hardcoded and its clients are lambdas, so no request is built and the date is inert. `preflight_miner.py` was **not** harmless: it builds real query params from that fixture and sends them to the Telegraph dispatcher, so after 2026-08-17T12:00Z it would fail on its own fixture's expired `forecast_cutoff`. That script exists to diagnose *dispatcher* problems on registration day, and **Track 1 opens 2026-08-17** — it would have broken in roughly the window it is needed, and the failure would have looked like a dispatcher fault. Now uses the same `rolling_horizon` helper (verified: cutoff +14.6 h, lead 15.6 h), with `--question-file` still pinning an exact question. Its `event_id` is also generated rather than inherited, because unlike the canary's this label is sent to Telegraph. 198 tests pass.

- **2026-08-10:** **The anchor X post is drafted and length-checked; publishing it is an operator action that is still owed.** `docs/x-update-drafts.md` §Draft 7 carries a 3-post thread and a single-post fallback, all under 280 (236 / 234 / 268-with-URL / 269 — counted rather than eyeballed, because the payload is a **64-character hash** and truncation would land squarely on it). Two content decisions are load-bearing. First, **the post publishes `receipt_count: 6` next to the head**: `--verify` recomputes over the *first N* rows, so a post carrying the hash alone commits to nothing anyone could check, and would have looked like evidence while being none. Second, **it labels the 6 receipts as our own canary and smoke traffic** — six receipts over six days is not adoption, and letting "6 receipts" read as six forecasts served would be the automated-traffic-as-demand claim the guardrails prohibit; self-published evidence is where that temptation is strongest because nobody else is checking the wording. **What the post does:** fixes the prefix at a timestamp OathCast cannot backdate, which genuinely binds us. **What it does not do:** let anyone verify. The store is on the Miner host and is not publicly readable, so no third party can recompute the chain — publication is a commitment, not a proof, and the draft says so in its own body. Closing that needs a **public read-only chain-head endpoint** (`chain_head()` already returns digests and counts only, never receipt content, so the handler is thin); it needs a redeploy, so it is queued for the next SSH window rather than justifying one.

- **2026-08-10:** **SSH window closed at ~20:45Z and verified from outside.** Full window 17:04:57Z → ~20:45Z; the security group is back to 80/443 only. Verified rather than assumed: TCP 22 filtered, **TCP 80 still reachable**, HTTPS `/healthz` 200 reporting `2026-08-10-hardened-v5`. The port-80 control is the load-bearing part — without it, "the SSH rule was removed" and "the host or the whole security group went dark" are indistinguishable. **A near-miss worth keeping:** the first check ran `timeout 15 bash -c ...`, but **macOS ships no `timeout`**, so the command exited 127 and the `|| echo closed` branch reported success — a green security result manufactured entirely by a missing binary. Re-run with `nc -z -G 5 -w 5`. The generalization is the same one the UID probe taught earlier today: a check that can pass because it failed to run is worse than no check, because it also stops you looking.
